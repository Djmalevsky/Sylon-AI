    ...
    <aside className="fixed left-0 top-0 z-50 flex h-screen w-[260px] flex-col border-r border-brand-500/15 bg-gradient-to-b from-brand-900 to-brand-950 text-white">
      ...
      <nav className="flex flex-1 flex-col gap-0.5 px-3 py-2">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className={cn("sidebar-link", isActive ? "sidebar-link-active" : "sidebar-link-inactive")}>
              <Icon size={18} />
              {item.label}
            </Link>
          );
      ...
      <div className="border-t border-white/10 px-5 py-4">
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[13px] font-semibold truncate">{userName || "Loading..."}</div>
            <div className="text-[11px] text-white/40 truncate">{userEmail}</div>
          </div>
          <button onClick={handleLogout} disabled={loggingOut} className="shrink-0 opacity-40 cursor-pointer hover:opacity-70 transition-opacity">
